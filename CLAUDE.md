...
| 日期 | AI | 改动内容 |
|------|-----|---------|
| 2026-06-28 | Claude Code | 首页看板优化: KPI4列横排+已发货/总计可点击; fmttime过滤器ISO转短格式(orders/order_detail统一); 低库存口径统一为active+inventory=0; 低库存提醒<10橙色档; DefaultTitle清洗; 待处理/今日暂不可点(订单页不支持多状态/日期筛选) |
